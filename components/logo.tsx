...
export const Logo = ({ className, imgClassName, ...props }: Props) => {
  const raw = env.NEXT_PUBLIC_LOGO_PATH;
  const isWebPath = typeof raw === 'string' && (raw.startsWith('/') || raw.startsWith('http'));
  const src = isWebPath ? raw : '/Easel-Logo.svg';
  const useImg = Boolean(src);

  if (useImg) {
    return (
      // Use <img> to allow dynamic path without static import
      // Tailwind sizing comes from className/imgClassName
      <img
        src={src}
