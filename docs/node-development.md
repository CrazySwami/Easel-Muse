...
Tersa uses a flow-based programming model where each node represents a specific functionality. The application is built on top of the React Flow (XYFlow) library, which provides the canvas and connection functionality.

### Core Components

Each node in Tersa consists of three main components:
...
For Tersa to recognize and use your new node, you need to register it in the central node type registry.
...
After these steps, your new node should be usable within the Tersa application.
...
Tersa nodes automatically operate in two distinct modes based on their connections:
...
Nodes in Tersa communicate by passing data through connections. Understanding how this data flows is crucial for building effective workflows.
...
- **Accessing Producer Node Data:** Each `incomer` in the array is a standard `Node` object. The data generated by the producer node is typically stored within its `data` property. A common convention in Tersa is to store generated output in a `generated` object within `data`.
...
- **Utility Functions:** Tersa provides helper functions like `getTextFromTextNodes` (in `lib/xyflow.ts`) to simplify extracting common data types from an array of incomers. You can create similar utilities for your specific data needs.
...
To handle this in a clean, maintainable, and scalable way, Tersa employs a pattern of using specialized helper functions located in `lib/xyflow.ts`.
...
Tersa includes several built-in node types, each designed for specific content types and workflows:
